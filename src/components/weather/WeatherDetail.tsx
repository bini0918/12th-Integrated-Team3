...
const StatusTag = memo(({ label, status }: WeatherTagData) => {
  const bgColor =
    status === "좋음" ? "bg-[#CCE8FF]"
    : status === "보통" ? "bg-[#CEFFCC]"
    : status === "나쁨" || status === "위험" ? "bg-[#FFCCCC]"
    : status.includes(":") ? "bg-[#F6FFCC]"
    : "bg-gray-200";

  const textColor =
    status === "좋음" ? "text-[#2D7DFF]"
    : status === "보통" ? "text-[#32FF35]"
    : status === "나쁨" || status === "위험" ? "text-[#FF3232]"
    : status.includes(":") ? "text-[#FFC532]"
    : "text-black";

  return (
    <div className={`flex flex-col w-32 px-4 py-2 rounded-xl text-sm text-center transition-transform hover:scale-[1.05] ${bgColor}`}>
...
const WeatherHeader = memo(({ icon, temperature, weatherStatus }: {
  icon: string;
  temperature: number;
  weatherStatus: string;
}) => (
